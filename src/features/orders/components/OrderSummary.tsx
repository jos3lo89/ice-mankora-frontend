...
export const OrderSummary = () => {
  ...
  const handleSend = () => {
    ...
    // Mapeo al formato del DTO del Backend
    const payload = {
      tableId,
  ...
  return (
    <Sheet>
      <SheetTrigger asChild>
        <Button className="fixed bottom-4 right-4 h-16 w-16 rounded-full shadow-xl z-50 animate-in zoom-in">
          <div className="relative">
            <ShoppingBasket size={28} />
            <span className="absolute -top-3 -right-3 bg-red-600 text-white text-xs font-bold h-6 w-6 rounded-full flex items-center justify-center border-2 border-white">
              {itemCount()}
            </span>
          </div>
        </Button>
